#!/bin/bash
# games_windows.sh - версия для Windows с открытием в новых окнах
...
while true; do
    ...
    case $choice in
        1) start bash -c "cd '$SCRIPT_DIR'; bash run_snake.sh" ;;
        2) start bash -c "cd '$SCRIPT_DIR'; bash run_hangman.sh" ;;
        3) start bash -c "cd '$SCRIPT_DIR'; bash run_dice.sh" ;;
        4) start bash -c "cd '$SCRIPT_DIR'; bash run_maze.sh" ;;
        5) start bash -c "cd '$SCRIPT_DIR'; bash run_tictactoe.sh" ;;
        6) start bash -c "cd '$SCRIPT_DIR'; bash run_pacman.sh" ;;
        7) 
